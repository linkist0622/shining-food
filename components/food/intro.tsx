...
export function BrandIntro({onFinish}:{onFinish:()=>void}){
 ...
 useEffect(()=>{
  ...
  const timers=[setTimeout(()=>setHeadlineVisible(true),3000),setTimeout(()=>setStage(1),2000),setTimeout(()=>setStage(2),4000),setTimeout(()=>setStage(3),6000),setTimeout(()=>setStage(4),9000),setTimeout(onFinish,10000)];
  return ()=>timers.forEach(clearTimeout);
 },[onFinish]);
 return <section className={`brand-intro stage-${stage}${headlineVisible?' headline-visible':''}`} aria-label="SHINING foodへようこそ">
